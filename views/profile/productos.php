        ...
        <?php foreach ($productos as $producto) { ?>
            <div class="product">
                ...
                <?php } else { ?>
                    ...
                    <div class="heart-container" data-product-id="<?php echo $producto->id; ?>">
...
<script>
    ...
    const heartStates = new Map();

    heartContainers.forEach(container => {
        const productId = container.dataset.productId;
        heartStates.set(productId, false);

        container.addEventListener('click', () => {
            const currentState = heartStates.get(productId);
            container.innerHTML = currentState ? emptyHeartIcon : heartIcon;
            heartStates.set(productId, !currentState);

            saveHeartState(productId, !currentState);
        });
    });

    function saveHeartState(productId, state) {

    }

    function loadSavedHeartStates() {
        heartContainers.forEach(container => {
            const productId = container.dataset.productId;
            const savedState = localStorage.getItem(`heart_${productId}`);

            if (savedState === 'true') {
                container.innerHTML = heartIcon;
                heartStates.set(productId, true);
            }
        });
    }

    loadSavedHeartStates();
</script>